...
// todo fix these
#define VENDOR_ID 0xa1a5u
#define PRODUCT_ID 0xad00u
...
void on_usb_configure(struct usb_device *device, bool configured)
{
    usb_configured = configured;
    if (configured)
    {
        gpio_put(25, 1); // 연결되면 LED ON
    }
    else
    {
        gpio_put(25, 0); // 연결 해제/리셋 시 LED OFF
    }
}
...
void usb_sound_card_init()
{
    ...
    assert(device);
    device->on_configure = on_usb_configure; // USB 연결 상태에 따라 LED 제어
